function is_word(word) {
    return words.includes(word) || possible_words.includes(word);
}
...
function msg_alert(msg, time) {
    let msgbox = document.querySelector("#alert"), spanbox = document.querySelector("#alert #alert-span");
    spanbox.textContent = msg;
    msgbox.animate([
        { top: "-12%" },
        { top: "0" }
    ], { duration: 1000, fill: "forwards", easing: "cubic-bezier(0, 1, 0.4, 1)" });
    setTimeout(() => {
        msgbox.animate([
            { top: "0" },
            { top: "-12%" }
        ], { duration: 1000, fill: "forwards", easing: "cubic-bezier(0, 1, 0.5, 1)" });
    }, time);
}
...
function close_all() {
    document.querySelectorAll('.absolute').forEach(el => { el.style.display = 'none'; });
}

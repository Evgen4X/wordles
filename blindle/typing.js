        ...
        if (row[0].getAttribute("index") == 5) {
            for (let j = 1; j < 6; j++) {
                ...
                for (let status of check) {
                    ...
                    let interval = (i - 1) * 200 + (j - 1) * 200 + 5000 + "ms";
                    letter.style.animation = `to_color 0.8s linear ${interval} 1 normal forwards`;
                    button.style.animation = `to_color 0s linear ${interval} 1 normal forwards`;
                    i++;
                }
            }
        }
        else if (row[0].getAttribute("index") != 6) {
...
if (params.get("word") == null) {
    ...
    msg_alert("That blindle may not use standart dictionary!", 7500);
}

...
If there're some words with the same amount of yellows/greens, choosing the first that occured
*/

function count_yellows(word, target) {
    let res = check_word(word, target), ans = 0;
    res.forEach(status => {
        if (status == 1) {
            ans++;
        }
...
function find_word(answer, checks) {
    let best_word = "", best_word_yellows = 6, best_word_greens = 6;
    var all_words = possible_words;
    if (global_dict) {
        all_words = words.concat(possible_words);
    }
    for (let i = 0; i < all_words.length; i++) {
        ...
        for (let check of checks) {
            for (let i = 1; i < check.length; i++) {
                let status = check[i], letter = check[0][i - 1];
